...
class Terrain(object):
    def __init__(self):
        ...
        self.w.show()
        self.w.setWindowTitle("Can't Stop Won't Stop")
        ...
        verts = np.array([
            [
                x, y, 1 + self.scale * self.tmp.noise2d(x=n / 5, y=m / 5)
        ...
        self.m1 = gl.GLMeshItem(
            vertexes=verts, drawFaces=False,
            faces=faces, edgeColor=(0.5, 0.5, 0., 1.),
    ...
    def update(self):
        ...
        verts = np.array([
            [
                x, y, 1 + self.scale * self.tmp.noise2d(x=n / 5 + self.offset, y=m / 5 + self.offset)
            ] for n, x in enumerate(self.xpoints) for m, y in enumerate(self.ypoints)
        ], dtype=np.float32)
